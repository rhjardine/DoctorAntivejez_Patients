...
/* ─── Category metadata ────────────────────────────────────────────── */
const CAT: Record<Category, { color: string; bg: string; border: string; label: string; text: string }> = {
    EXCELENTE: { color: '#5A7163', bg: 'rgba(90,113,99,0.1)', border: 'rgba(90,113,99,0.3)', label: 'Excelente Vitalidad', text: 'text-vytalix-sage' },
    BUENO: { color: '#5A7163', bg: 'rgba(90,113,99,0.1)', border: 'rgba(90,113,99,0.3)', label: 'Buena Condición', text: 'text-vytalix-sage' },
    REGULAR: { color: '#B35446', bg: 'rgba(179,84,70,0.1)', border: 'rgba(179,84,70,0.3)', label: 'Área de Oportunidad', text: 'text-vytalix-terracotta' },
    CRITICO: { color: '#B35446', bg: 'rgba(179,84,70,0.1)', border: 'rgba(179,84,70,0.3)', label: 'Atención Prioritaria', text: 'text-vytalix-terracotta' },
};
...
const ResultadoScorePage: React.FC = () => {
    ...
    const hookConfig = score < 50
        ? {
            color: '#B35446', icon: <AlertTriangle size={20} />,
            title: 'Tu score indica áreas de atención prioritaria',
            rangeBadge: 'Optimización Activa',
            context: 'Tu biología está respondiendo a patrones que aceleran el envejecimiento celular. El 70% de estos factores son modificables con el protocolo correcto.',
            opportunity: 'Una evaluación clínica puede identificar exactamente cuáles son tus prioridades de mayor impacto vital.',
        }
        : {
            color: '#5A7163', icon: <ShieldCheck size={20} />,
            title: 'Tu score refleja una base sólida de vitalidad',
            rangeBadge: 'Rango de Excelencia',
            context: 'Tus hábitos actuales están funcionando. La diferencia entre salud "estándar" y longevidad "premium" está en la precisión clínica.',
            opportunity: 'El protocolo personalizado puede consolidar este estado y preservarlo 15–20 años más allá del promedio biológico.',
        };
            ...
            <div className="w-full max-w-[420px] px-6 pt-12">
                ...
                >
                    <div className="relative flex flex-col items-center mb-4">
                        <SemiGauge score={score} color={score >= 60 ? '#5A7163' : '#B35446'} />
                        ...
                        <div className="mt-8 p-5 bg-white rounded-3xl border border-vytalix-terracotta/20 animate-pulse-subtle">
                            <div className="flex items-start gap-4 mb-4">
                                ...
                                <p className="text-[13px] font-medium leading-relaxed text-vytalix-graphite/70">
                                    Este score es una <strong className="text-vytalix-terracotta">estimación biométrica facial</strong>. Realiza el test clínico para precisión total.
                                </p>
                            </div>
                ...
                >
                    <div className="flex items-start gap-4 mb-6">
                        <div className="shrink-0" style={{ color: hookConfig.color }}>{hookConfig.icon}</div>
                        <div>
                            <p className="text-lg font-black leading-tight text-vytalix-graphite tracking-tight mb-2">{hookConfig.title}</p>
                            <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full bg-vytalix-sand border border-vytalix-graphite/5"
                            ...
                            return (
                                <div key={d.key}>
                                    <div className="flex justify-between items-center mb-3">
                                        <span className="text-[13px] font-bold text-vytalix-graphite tracking-tight">{d.label}</span>
                    ...
                    <AnimatePresence mode="wait">
                        {!alreadyCaptured && leadStatus !== 'sent' ? (
                            <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                onSubmit={handleLead} className="relative z-10">
                                <p className="text-xl font-black mb-3 tracking-tight">Reporte de Bio-Optimización</p>
                                <p className="text-xs mb-8 text-white/50 leading-relaxed font-medium uppercase tracking-widest">
                                    Enviamos tu protocolo inicial y el desglose de tus 5 dimensiones clínicas.
                                </p>
                                ...
                                <button type="submit" disabled={leadStatus === 'sending'}
                                    ...
                                    {leadStatus === 'sending' ? <Loader2 className="animate-spin mx-auto" /> : 'ACTIVAR PROTOCOLO'}
                                </button>
                            </motion.form>
                        ) : (
                            <motion.div key="success" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-6 relative z-10">
                                <CheckCircle size={56} className="text-vytalix-sage mx-auto mb-6" />
                                <p className="text-xl font-black mb-2 uppercase tracking-tighter">Acceso Concedido</p>
                                <p className="text-[13px] text-white/50 font-medium">Tu reporte clínico está en camino.</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </motion.div>
                ...
                {/* ── BLOQUE 5: Next Steps ── */}
                {(alreadyCaptured || leadStatus === 'sent') && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 mb-12">
                        ...
                        <button onClick={() => setSelectedTier('basica')}
                            className={`w-full text-left p-6 rounded-[2rem] transition-all border-2 ${selectedTier === 'basica' ? 'border-vytalix-sage bg-vytalix-sage/5' : 'border-vytalix-graphite/5 bg-white'
                                }`}>
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-[9px] font-black uppercase px-2 py-1 rounded bg-vytalix-sand text-vytalix-graphite/40 tracking-widest">Inversión: $0</span>
                                {selectedTier === 'basica' && <CheckCircle className="text-vytalix-sage" size={20} />}
                            </div>
                            <p className="text-base font-black text-vytalix-graphite tracking-tight">Review de Optimización</p>
                            <p className="text-xs mt-1 text-vytalix-graphite/50 font-medium leading-relaxed">Definición de tus 2 prioridades críticas con nuestro equipo médico.</p>
                        ...
                        <button onClick={() => setSelectedTier('profunda')}
                            className={`w-full text-left p-6 rounded-[2rem] transition-all border-2 relative overflow-hidden ${selectedTier === 'profunda' ? 'border-vytalix-terracotta bg-vytalix-terracotta/5' : 'border-vytalix-graphite/5 bg-white'
                                }`}>
                            <div className="absolute top-0 right-0 py-1 px-4 bg-vytalix-terracotta text-white text-[8px] font-black uppercase tracking-[0.3em] rounded-bl-xl">Protocolo Full</div>
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-[9px] font-black uppercase px-2 py-1 rounded bg-vytalix-terracotta/10 text-vytalix-terracotta tracking-widest">Vytalix Premium</span>
                                {selectedTier === 'profunda' && <CheckCircle className="text-vytalix-terracotta" size={20} />}
                            </div>
                            <p className="text-base font-black text-vytalix-graphite tracking-tight">Evaluación de Edad Celular</p>
                            <p className="text-xs mt-1 text-vytalix-graphite/50 font-medium leading-relaxed">Evaluación biofísica multimodal + Plan maestro de longevidad.</p>
                ...
                <div className="text-center pb-12">
                    <p className="text-[10px] font-semibold text-vytalix-graphite/30 uppercase tracking-[0.4em] mb-4">VYTALIX Longevity Suite</p>
                    <button onClick={() => navigate('/login')} className="text-xs font-black uppercase tracking-widest text-vytalix-graphite/50 underline decoration-vytalix-graphite/10">Portal Médico →</button>
                </div>
            </div>
        </div>
    );
