...
const NutrigenomicsView: React.FC<NutrigenomicsViewProps> = ({ onBack }) => {
  ...
  const derivedForbidden = useMemo(() => {
    if (!plan) return [];
    const bloodRules = BLOOD_TYPE_RESTRICTIONS[plan.bloodType] || [];
         ...
         {(['BREAKFAST', 'LUNCH', 'DINNER', 'SNACK'] as MealType[]).map((tab) => {
             ...
             return (
               <button
                  ...
                  className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-[10px] font-black transition-all border-2 ${
                      isActive 
                      ...
                      <div className="grid grid-cols-1 gap-3">
                          {groupedFoods[category].map((food) => {
                              ...
                              return (
                                <div 
                                    key={food.id} 
                                    className={`relative bg-white rounded-[1.75rem] p-5 shadow-sm border-2 transition-all flex items-center gap-4 ${
                                      food.isClinicalPriority ? 'border-primary/20 bg-sky-50/20' : 'border-white hover:border-slate-100'
                                    }`}
                                >
                                    ...
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner shrink-0 ${
                                      food.isClinicalPriority ? 'bg-primary text-white' : 'bg-slate-50 text-primary'
                                    }`}>
                                    ...
                                    <button 
                                      ...
                                      className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                                          isFav ? 'bg-red-500 text-white shadow-lg' : 'bg-slate-50 text-slate-200'
                                      }`}
                                    >
