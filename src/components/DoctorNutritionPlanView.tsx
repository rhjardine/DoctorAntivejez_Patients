...
import {
    ...
    Dna, BookOpen, Heart, Activity, Zap
} from 'lucide-react';
...
const DoctorNutritionPlanView: React.FC<Props> = ({ onBack }) => {
    ...
    const alimentosEvitar = useMemo(() =>
        alimentacion?.alimentosEvitar ? alimentacion.alimentosEvitar.split('\n') : DEFAULTS_COMUNES.alimentosEvitar
        , [alimentacion]);

    const sustitutos = useMemo(() =>
        alimentacion?.sustitutos ? alimentacion.sustitutos.split('\n') : DEFAULTS_COMUNES.sustitutos
        , [alimentacion]);

    const claves5a = useMemo(() => alimentacion?.claves5a || DEFAULTS_COMUNES.claves5a, [alimentacion]);
    const terapias4r = useMemo(() => alimentacion?.terapias4r || DEFAULTS_COMUNES.terapias4r, [alimentacion]);
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-5 duration-500">
                        ...
                        <div className="px-5 space-y-3 pb-10">
                            {(planAlimentario[activeMeal] || []).map((item, idx) => (
                                <div key={idx} className="bg-white rounded-[1.5rem] p-5 shadow-sm border border-slate-100 flex items-center gap-4 group">
                                    <div className="w-10 h-10 rounded-2xl bg-[#23BCEF]/10 flex items-center justify-center shrink-0 group-hover:bg-[#23BCEF] transition-colors">
                                        <CheckCircle2 size={18} className="text-[#23BCEF] group-hover:text-white" />
                                    </div>
                                    <span className="text-sm font-bold text-[#293b64] leading-tight">{item}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="px-5 pt-6 space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-5 duration-500">
                        ...
                        <div className="bg-red-50 border-2 border-red-100 rounded-[2.5rem] p-6 shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-[11px] font-black text-red-700 uppercase tracking-widest">Incompatibles</h4>
                            </div>
                            <div className="space-y-2">
                                {alimentosEvitar.map((item, i) => (
                                    <div key={i} className="flex gap-2 text-xs text-red-900 font-bold leading-relaxed opacity-90">
                        ...
                        <div className="bg-emerald-50 border-2 border-emerald-100 rounded-[2.5rem] p-6 shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-[11px] font-black text-emerald-700 uppercase tracking-widest">Sustitutos Sanos</h4>
                            </div>
                            <div className="space-y-2">
                                {sustitutos.map((item, i) => (
                                    <div key={i} className="flex gap-2 text-xs text-emerald-900 font-bold leading-relaxed opacity-90">
                ...
                {activeSection === 'claves' && (
                    <div className="px-5 pt-6 space-y-4 pb-20 animate-in fade-in slide-in-from-bottom-5 duration-500">
                        {claves5a.map((clave: any, idx: number) => (
                            <div key={idx} className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex flex-col gap-4">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-[#23BCEF]/10 flex items-center justify-center text-xl shadow-inner">
                                        {clave.icono}
                                    </div>
                                    <div>
                                        <h4 className="text-xs font-black text-[#293b64] uppercase tracking-widest">{clave.clave}</h4>
                                        <div className="h-1 w-8 bg-[#23BCEF] rounded-full mt-1 opacity-50" />
                                    </div>
                                </div>
                                <div className="space-y-2.5">
                                    {clave.items.map((item: string, i: number) => (
                                        <div key={i} className="flex gap-3 text-xs font-bold text-slate-500 leading-relaxed pl-2">
                                            <span className="text-[#23BCEF] mt-0.5 whitespace-nowrap">🔸</span>
                                            {item}
                                        </div>
                                    ))}
                                </div>
                ...
                {activeSection === 'terapias' && (
                    <div className="px-5 pt-6 space-y-5 pb-20 animate-in fade-in slide-in-from-bottom-5 duration-500">
                        {terapias4r.map((t: any, idx: number) => (
                            <div key={idx} className="bg-white rounded-[2.5rem] p-7 border border-slate-100 shadow-sm relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-1.5 h-full bg-[#23BCEF]" />
                                <div className="mb-4">
                                    <h3 className="text-sm font-black text-[#293b64] uppercase tracking-tight">{t.nombre}</h3>
                                    <p className="text-[11px] font-black text-[#23BCEF] italic mt-0.5 opacity-80">"{t.slogan}"</p>
                                </div>
                                <ul className="space-y-3">
                                    {t.items.map((item: string, i: number) => (
                                        <li key={i} className="flex gap-3 text-xs font-bold text-slate-600 bg-slate-50/80 p-3 rounded-2xl border border-slate-100">
